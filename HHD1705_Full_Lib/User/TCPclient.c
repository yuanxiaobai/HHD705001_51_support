...
void tcp_echoclient_connect(void)
{
  ...
  {
    IP4_ADDR( &DestIPaddr, 192, 168, 3, 26 );
    
    tcp_bind(echoclient_pcb, IP_ADDR_ANY, 0); 		
    /* connect to destination address/port */
    tcp_connect(echoclient_pcb,&DestIPaddr,100,tcp_Client_connected);
...
#if LCD_USER
	LCD_ShowString(0, 18*9, "Send Date:", LGRAY, BLACK);	
	LCD_ShowString(20, 18*10, data, RED, BLACK); 
#endif
