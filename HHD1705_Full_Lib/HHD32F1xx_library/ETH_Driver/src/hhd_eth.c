...
extern void Delay_1ms(uint32_t nCount);
...
void PHY_MII_TO_SGMII(uint16_t PHYAddress)
{
	uint32_t RegValue = 0;
	...
	RegValue &= 0xFFFF;
	RegValue |= 0x800E;           //27.15 = 1,disable fiber/copper auto-selection
	ETH_WritePHYRegister(PHYAddress, 27, RegValue); 
	Delay_1ms(1000);
	...
	RegValue &= 0xFFFF;
	RegValue |= 0x1;
	ETH_WritePHYRegister(PHYAddress, 22, RegValue);
	Delay_1ms(1000);

	// set smgii speed
	RegValue = ETH_ReadPHYRegister(PHYAddress, 20);
	RegValue &= 0xFF8F;
	RegValue |= 0x0050;           //100M,TX_CLK link_speed 25MH
	ETH_WritePHYRegister(PHYAddress, 20, RegValue);
...
void PHY_MII_COPPER(uint16_t PHYAddress)
{
	...
	RegValue = RegValue|0x000F;   //(F)1111:MII to copper;
	ETH_WritePHYRegister(PHYAddress, 27, RegValue);
...
//	RegValue = RegValue|0x1000 ;    // reg0_bit.12 =1  自适应模式
	RegValue = RegValue&0xFFBF ;    // reg0_bit.6  =0
#ifdef ETH_100M 
	RegValue = RegValue|0x2000 ;	// reg0_bit.13 =1 时是100 M
#else 	
	RegValue = RegValue&0xDFFF ;	  // reg0_bit.13 =0 时是10 M
#endif
...
//	RegValue &= 0xFF8F;
//#ifdef ETH_100M 
//	RegValue = RegValue|0x0050 ;	// 100 M
//#else 	
//	RegValue = RegValue|0x0040 ;	  // 10 M
//#endif
...
//	RegValue = ETH_ReadPHYRegister(PHYAddress, 0); 
//	RegValue |= 0x4000;
//	ETH_WritePHYRegister(PHYAddress, 0, RegValue);//bit.14 loopback
...
/**
  ...
  */
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress, EN_NET_PATH path)
//uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  uint32_t  tmpreg = 0;
  __IO uint32_t i = 0;
//	uint32_t hclk = 75000000;
...
//  }
  ...
  /* Write to ETHERNET MAC MIIAR: Configure the ETHERNET CSR Clock Range */
    HHD_ETH->MACMIIAR = (uint32_t)tmpreg;
  /*-------------------- PHY initialization and configuration ----------------*/
	...
	if(path == EN_TO_SWITCH)
	{
		PHY_MII_TO_SGMII(PHYAddress);
	}
	else
	{
		PHY_MII_COPPER(PHYAddress);
	}
    ...
    HHD_ETH->MACFFR = (uint32_t)(ETH_InitStruct->ETH_ReceiveAll |
    ...
    HHD_ETH->MACHTHR = (uint32_t)ETH_InitStruct->ETH_HashTableHigh;
    /* Write to ETHERNET MACHTLR */
    HHD_ETH->MACHTLR = (uint32_t)ETH_InitStruct->ETH_HashTableLow;
    /*----------------------- ETHERNET MACFCR Configuration --------------------*/
    /* Get the ETHERNET MACFCR value */
    tmpreg = HHD_ETH->MACFCR;
    /* Clear xx bits */
    ...
    HHD_ETH->MACFCR = (uint32_t)tmpreg;
    /*----------------------- ETHERNET MACVLANTR Configuration -----------------*/
    /* Set the ETV bit according to ETH_VLANTagComparison value */
    /* Set the VL bit according to ETH_VLANTagIdentifier value */
    HHD_ETH->MACVLANTR = (uint32_t)(ETH_InitStruct->ETH_VLANTagComparison |
                                    ETH_InitStruct->ETH_VLANTagIdentifier);

    /*-------------------------------- DMA Config ------------------------------*/
    /*----------------------- ETHERNET DMAOMR Configuration --------------------*/
    /* Get the ETHERNET DMAOMR value */
    tmpreg = HHD_ETH->DMAOMR;
    /* Clear xx bits */
    ...
    HHD_ETH->DMAOMR = (uint32_t)tmpreg;
    ...
    HHD_ETH->DMABMR = (uint32_t)(ETH_InitStruct->ETH_AddressAlignedBeats |
...
uint32_t ETH_HandleTxPkt(uint8_t *ppkt, uint16_t FrameLength)
{
    ...
    if ((HHD_ETH->DMASR & ETH_DMASR_TBUS) != (uint32_t)RESET)
    {
    ...
    else /* Ring Mode */
    {
        if((DMATxDescToSet->ControlBufferSize & ETH_DMATxDesc_TER) != (uint32_t)RESET)
        {
            ...
            DMATxDescToSet = (ETH_DMADESCTypeDef *) (HHD_ETH->DMATDLAR);
        }
        else
        {
            /* Selects the next DMA Tx descriptor list for next buffer to send */
            DMATxDescToSet = (ETH_DMADESCTypeDef *) ((uint32_t)DMATxDescToSet + 0x10 + ((
                                 HHD_ETH->DMABMR & ETH_DMABMR_DSL) >> 2));
        }
    }
    /* Return SUCCESS */
    return ETH_SUCCESS;
...
uint16_t ETH_ReadPHYRegister(uint16_t PHYAddress, uint16_t PHYReg)
{
    ...
    return (uint16_t)(HHD_ETH->MACMIIDR);
}
...
FlagStatus ETH_GetFlowControlBusyStatus(void)
{
    ...
    if ((HHD_ETH->MACFCR & ETH_MACFCR_FCBBPA) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
void ETH_InitiatePauseControlFrame(void)
{
    ...
    HHD_ETH->MACFCR |= ETH_MACFCR_FCBBPA;
}
...
FlagStatus ETH_GetMACFlagStatus(uint32_t ETH_MAC_FLAG)
{
    ...
    if ((HHD_ETH->MACSR & ETH_MAC_FLAG) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
ITStatus ETH_GetMACITStatus(uint32_t ETH_MAC_IT)
{
    ...
    if ((HHD_ETH->MACSR & ETH_MAC_IT) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
void ETH_MACITConfig(uint32_t ETH_MAC_IT, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        HHD_ETH->MACIMR &= (~(uint32_t)ETH_MAC_IT);
    }
    else
    {
        /* Disable the selected ETHERNET MAC interrupts */
        HHD_ETH->MACIMR |= ETH_MAC_IT;
    }
}
...
void ETH_DMATxDescChainInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t *TxBuff, uint32_t TxBuffCount)
{
    ...
    HHD_ETH->DMATDLAR = (uint32_t) DMATxDescTab;
}
...
/**
  ...
  * @retval None
  */
void ETH_DMATxDescRingInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t *TxBuff1, uint8_t *TxBuff2,
...
FlagStatus ETH_GetDMATxDescFlagStatus(ETH_DMADESCTypeDef *DMATxDesc, uint32_t ETH_DMATxDescFlag)
{
    ...
    if(ETH_DMATxDescFlag < (1 <<
                            18))		//since MAC version upgrade from 3.5 to 3.7, Descriptor structure are changed
    {
    ...
    {
        if ((DMATxDesc->ControlBufferSize & ETH_DMATxDescFlag) != (uint32_t)RESET)
        {
            bitstatus = SET;
        }
        else
...
/**
  ...
  * @retval None
  */
void ETH_DMATxDescBufferSizeConfig(ETH_DMADESCTypeDef *DMATxDesc, uint32_t BufferSize1,
...
void ETH_DMARxDescChainInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff, uint32_t RxBuffCount)
{
    ...
    HHD_ETH->DMARDLAR = (uint32_t) DMARxDescTab;
}
...
/**
  ...
  * @retval None
  */
void ETH_DMARxDescRingInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff1, uint8_t *RxBuff2,
...
{
    FlagStatus bitstatus = RESET;
    if((HHD_ETH->DMABMR & ETH_DMABMR_SR) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
FlagStatus ETH_GetDMAFlagStatus(uint32_t ETH_DMA_FLAG)
{
    ...
    if ((HHD_ETH->DMASR & ETH_DMA_FLAG) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
void ETH_DMAClearFlag(uint32_t ETH_DMA_FLAG)
{
    ...
    HHD_ETH->DMASR = (uint32_t) ETH_DMA_FLAG;
}
...
ITStatus ETH_GetDMAITStatus(uint32_t ETH_DMA_IT)
{
    ...
    if ((HHD_ETH->DMASR & ETH_DMA_IT) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
void ETH_DMAClearITPendingBit(uint32_t ETH_DMA_IT)
{
    ...
    HHD_ETH->DMASR = (uint32_t) ETH_DMA_IT;
}
...
{
    return ((uint32_t)(HHD_ETH->DMASR & ETH_DMASR_TS));
}
...
{
    return ((uint32_t)(HHD_ETH->DMASR & ETH_DMASR_RS));
}
...
{
    FlagStatus bitstatus = RESET;
    if ((HHD_ETH->DMAOMR & ETH_DMAOMR_FTF) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
void ETH_DMAITConfig(uint32_t ETH_DMA_IT, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        HHD_ETH->DMAIER |= ETH_DMA_IT;
    }
    else
    {
        /* Disable the selected ETHERNET DMA interrupts */
        HHD_ETH->DMAIER &= (~(uint32_t)ETH_DMA_IT);
    }
}
...
FlagStatus ETH_GetDMAOverflowStatus(uint32_t ETH_DMA_Overflow)
{
    ...
    if ((HHD_ETH->DMAMFBOCR & ETH_DMA_Overflow) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
{
    return ((uint32_t)((HHD_ETH->DMAMFBOCR & ETH_DMAMFBOCR_MFA) >>
...
{
    return ((uint32_t)(HHD_ETH->DMAMFBOCR) & ETH_DMAMFBOCR_MFC);
}
...
{
    return ((uint32_t)(HHD_ETH->DMACHTDR));
}
...
{
    return ((uint32_t)(HHD_ETH->DMACHRDR));
}
...
{
    return ((uint32_t)(HHD_ETH->DMACHTBAR));
}
...
{
    return ((uint32_t)(HHD_ETH->DMACHRBAR));
}
...
void ETH_ResetWakeUpFrameFilterRegisterPointer(void)
{
    ...
    HHD_ETH->MACPMTCSR |= ETH_MACPMTCSR_WFFRPR;
}
...
void ETH_GlobalUnicastWakeUpCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        HHD_ETH->MACPMTCSR |= ETH_MACPMTCSR_GU;
    }
    else
    {
...
FlagStatus ETH_GetPMTFlagStatus(uint32_t ETH_PMT_FLAG)
{
    ...
    if ((HHD_ETH->MACPMTCSR & ETH_PMT_FLAG) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
void ETH_WakeUpFrameDetectionCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        HHD_ETH->MACPMTCSR |= ETH_MACPMTCSR_WFE;
    }
    else
    {
...
void ETH_MagicPacketDetectionCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        HHD_ETH->MACPMTCSR |= ETH_MACPMTCSR_MPE;
    }
    else
    {
...
void ETH_PowerDownCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        HHD_ETH->MACPMTCSR |= ETH_MACPMTCSR_PD;
    }
    else
    {
...
void ETH_MMCITConfig(uint32_t ETH_MMC_IT, FunctionalState NewState)
{
    ...
    if ((ETH_MMC_IT & (uint32_t)0x10000000) != (uint32_t)RESET)
    {
        ...
        if (NewState != DISABLE)
        {
            ...
            HHD_ETH->MMCRIMR &= (~(uint32_t)ETH_MMC_IT);
        }
        else
        {
            /* Disable the selected ETHERNET MMC interrupts */
            HHD_ETH->MMCRIMR |= ETH_MMC_IT;
        }
    }
    else
    {
        ...
        {
            /* Enable the selected ETHERNET MMC interrupts */
            HHD_ETH->MMCTIMR &= (~(uint32_t)ETH_MMC_IT);
        }
        else
        {
            /* Disable the selected ETHERNET MMC interrupts */
            HHD_ETH->MMCTIMR |= ETH_MMC_IT;
        }
    }
}
...
ITStatus ETH_GetMMCITStatus(uint32_t ETH_MMC_IT)
{
    ...
    if ((ETH_MMC_IT & (uint32_t)0x10000000) != (uint32_t)RESET)
    {
        ...
        if ((((HHD_ETH->MMCRIR & ETH_MMC_IT) != (uint32_t)RESET))
                && ((HHD_ETH->MMCRIMR & ETH_MMC_IT) != (uint32_t)RESET))
        {
            bitstatus = SET;
        }
        else
    ...
    else
    {
        ...
        if ((((HHD_ETH->MMCTIR & ETH_MMC_IT) != (uint32_t)RESET))
                && ((HHD_ETH->MMCRIMR & ETH_MMC_IT) != (uint32_t)RESET))
        {
            bitstatus = SET;
        }
...
void ETH_EnablePTPTimeStampAddend(void)
{
    ...
    HHD_ETH->PTPTSCR |= ETH_PTPTSCR_TSARU;
}
...
void ETH_EnablePTPTimeStampInterruptTrigger(void)
{
    ...
    HHD_ETH->PTPTSCR |= ETH_PTPTSCR_TSITE;
}
...
void ETH_EnablePTPTimeStampUpdate(void)
{
    ...
    HHD_ETH->PTPTSCR |= ETH_PTPTSCR_TSSTU;
}
...
void ETH_InitializePTPTimeStamp(void)
{
    ...
    HHD_ETH->PTPTSCR |= ETH_PTPTSCR_TSSTI;
}
...
void ETH_PTPUpdateMethodConfig(uint32_t UpdateMethod)
{
    ...
    if (UpdateMethod != ETH_PTP_CoarseUpdate)
    {
        ...
        HHD_ETH->PTPTSCR |= ETH_PTPTSCR_TSFCU;
    }
    else
    {
        /* Disable the PTP Coarse Update method */
        HHD_ETH->PTPTSCR &= (~(uint32_t)ETH_PTPTSCR_TSFCU);
    }
}
...
void ETH_PTPTimeStampCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        HHD_ETH->PTPTSCR |= ETH_PTPTSCR_TSE;
    }
    else
    {
        /* Disable the PTP time stamp for transmit and receive frames */
        HHD_ETH->PTPTSCR &= (~(uint32_t)ETH_PTPTSCR_TSE);
    }
}
...
FlagStatus ETH_GetPTPFlagStatus(uint32_t ETH_PTP_FLAG)
{
    ...
    if ((HHD_ETH->PTPTSCR & ETH_PTP_FLAG) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
void ETH_SetPTPSubSecondIncrement(uint32_t SubSecondValue)
{
    ...
    HHD_ETH->PTPSSIR = SubSecondValue;
}
...
void ETH_SetPTPTimeStampUpdate(uint32_t Sign, uint32_t SecondValue, uint32_t SubSecondValue)
{
    ...
    HHD_ETH->PTPTSHUR = SecondValue;

    /* Set the PTP Time Update Low Register with sign */
    HHD_ETH->PTPTSLUR = Sign | SubSecondValue;
}
...
void ETH_SetPTPTargetTime(uint32_t HighValue, uint32_t LowValue)
{
    ...
    HHD_ETH->PTPTTHR = HighValue;
    /* Set the PTP Target Time Low Register */
    HHD_ETH->PTPTTLR = LowValue;
}
    ...
    *PTPTxTab = DMATxDescToSet->Buffer2NextDescAddr;
    ...
    else /* Ring Mode */
    {
        if((DMATxDescToSet->ControlBufferSize & ETH_DMATxDesc_TER) != (uint32_t)RESET)
        {
            ...
            DMATxDescToSet = (ETH_DMADESCTypeDef *) (HHD_ETH->DMATDLAR);
            DMAPTPTxDescToSet = (ETH_DMADESCTypeDef *) (HHD_ETH->DMATDLAR);
        }
        else
        {
            /* Selects the next DMA Tx descriptor list for next buffer read */
...
uint32_t ETH_GetTxDescStat()
{
	return DMATxDescToSet->Status;
}
uint32_t ETH_GetRxDescStat()
{
	return DMAPTPRxDescToGet->Status;
}
